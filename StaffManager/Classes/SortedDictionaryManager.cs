...
public partial class SortedDictionaryManager: ObservableObject {
    ...
    private void StaffDataPropertyChanged (object? sender, PropertyChangedEventArgs e){
        switch (e.PropertyName){
            ...
            case nameof(StaffData.SelectedStaffMember):
                ...
                break;
            default:
                UserFeedback.DisplayWarning($"Unhandled property change: {e.PropertyName}", "Unexpected Property");
                break;
        }
    }
    ...
    private void OnSelectedStaffMemberChanged (StaffMember? value){
        if (value != null){
            ...
            UserFeedback.DisplayErrorMessage("No staff member was selected; cannot update name or ID fields.", "No Value");
        }
    }
    ...
    private void InitaliseNewEntry (){
        if (StaffData.MasterFile.SortedData == null || string.IsNullOrWhiteSpace(StaffData.NewStaffName)){
    ...
    [RelayCommand] private void AddNewStaffMember () => InitaliseNewEntry();
}